...
namespace VRText.SharpOSC
{
	public abstract class OscPacket
	{
		...
		private static VRText.SharpOSC.OscMessage parseMessage(byte[] msg)
		{
			int index = 0;
			...
			foreach (char type in types)
			{
				...
				switch(type)
				{
					...
					case ('m'):
						VRText.SharpOSC.Midi midival = getMidi(msg, index);
			...
			return new VRText.SharpOSC.OscMessage(address, arguments.ToArray());
		}
		...
		private static VRText.SharpOSC.OscBundle parseBundle(byte[] bundle)
		{
			UInt64 timetag;
			List<VRText.SharpOSC.OscMessage> messages = new List<VRText.SharpOSC.OscMessage>();
			...
			VRText.SharpOSC.OscBundle output = new VRText.SharpOSC.OscBundle(timetag, messages.ToArray());
			return output;
		}
		...
		private static VRText.SharpOSC.Midi getMidi(byte[] msg, int index)
		{
			return new VRText.SharpOSC.Midi(msg[index], msg[index + 1], msg[index + 2], msg[index + 3]);
		}
		...
		protected static byte[] setString(string value)
		{
			...
			var len = utf8Encoding.GetByteCount(value) + (4 - value.Length % 4);
		...
		protected static byte[] setMidi(VRText.SharpOSC.Midi value)
		{
			byte[] output = new byte[4];
			output[0] = value.Port;
